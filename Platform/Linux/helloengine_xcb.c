...
//在Linux当中，图形界面是交给一个叫做Xserver的服务进行绘制管理，而客户端程序实际上并不直接绘制画面，
//而是生成一系列画面绘制指令，通过一种事先定义好的协议（X协议），
//发送给Xserver。Xserver根据这些绘图指令进行实际的绘图。
//打个小小的比方，X协议就好比HTML，而Xserver就好比浏览器。
